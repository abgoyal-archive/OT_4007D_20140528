 ...
 * -------
 *   James Lo 		(MTK02485)
 * 20130416 fix FR440426 by sunjiaojiao 
 ****************************************************************************/
    ...
    else if ((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == CHARGING_HOST))
    {
         if (BMT_status.temperature >= BATTERY_TEMPERATURE_P42)
			...
			g_temp_CC_value = CHARGER_CURRENT_400;
		      if (Enable_BATDRV_LOG == 1) {
                     printf("[BATTERY] STANDARD_CHARGER or CHARGING_HOST CC mode charging : %d\r\n", CHARGER_CURRENT_400);
                   }  
         	}
...
int BAT_CheckBatteryStatus(void)
{
    ...
    if (low_bat_boot_display == 0)
    {
      ...
      //20120921 modify by sunjiaojiao start
       if((bat_volt_check_point<100)&&(upmu_is_chr_det() == KAL_TRUE))
       	{
