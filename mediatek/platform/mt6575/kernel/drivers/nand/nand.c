...
//PR532576  added by yongming.li for   imei  refurbish  begin
#include <linux/kernel.h>
#include <linux/syscalls.h>
#include <linux/fcntl.h>
#include <asm/segment.h>
#include <linux/buffer_head.h>
//PR532576  added by yongming.li for   imei  refurbish  end
...
static void id_offset(struct otp_ctl *p)
{
     ...
     if(fd >= 0) 
       {
            ...
            sys_close(fd);
            printk(KERN_DEBUG "[EMMC_OTP] successful  open %s file and loops is %d   shit \n", TRACE_REGION,loops++);
            }
       else 
      {
               printk(KERN_DEBUG "[EMMC_OTP]can not open %s file and loops is %d   and fd is %d  and proc name is %s  \n", TRACE_REGION,loops++,fd,current->comm);
               goto fail;
      }
    ...
    set_fs(oldfs);
    printk(KERN_DEBUG "[EMMC_OTP] dump pro_info index from 40\n");
    dumpBuf(buffer+40);
...
ready:
	  //  for test begin
	  //  g_id_offset.group = 0x01;
	  //  for test end
    p->Offset = g_id_offset.group*0x80+(g_id_offset.offset/0xA)*0x20;
    printk(KERN_ERR "otp use id offset 0x%x\n", p->Offset);
...
static long mt_otp_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    //PR532576  added by yongming.li for   imei  refurbish  
    sys_chmod(TRACE_REGION,0666);
    id_offset(&otpctl);
    printk(KERN_ERR "[%s]:  id_offset(&otpctl);  \n", __func__);
    printk(KERN_ERR "[%s]:  OTP_PAGE_OFFSET is %d  \n", __func__,OTP_PAGE_OFFSET);
