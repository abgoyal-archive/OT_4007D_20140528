 ...
 *
 ****************************************************************************/
//2012-10-29 modify by sunjiaojiao,changed CV_VTH to 4.1875V and cut-off current to 200mA
//2012-12-13 modify by sunjiaojiao,add function for stoping charging when battery id error
...
/*susheng.ye.hz add for FR468744 20130614 begin */
#if defined(TARGET_BUILD_MMITEST)
#include <mach/mt6575_boot.h>
#endif
/*susheng.ye.hz add for FR468744 20130614 end */
...
#define BATTERY_OVER_TEMP 3  //20130604 add for 43 and 2 degree recharge
/*add by sunjiaojiao for FR373645 start*/
...
#endif
/*add by sunjiaojiao for FR373645 end*/
///////////////////////////////////////////////////////////////////////////////////////////
...
typedef struct 
{
    kal_bool   	bat_exist;
#if BATT_ID_CHECK_SUPPORT
    kal_bool   	bat_id_valid;// by sunjiaojiao for FR373645 
#endif
    kal_bool   	bat_full;  
    kal_bool   	bat_low;  
...
int CHARGING_FULL_CURRENT=200;	// mA on phone 220 changed to 200
...
//cut off to full
#define POST_CHARGING_TIME 15 * 60 // 30mins//20130618 wangxingxing changed for fixing FR471119
...
void select_charging_curret(void)
{
    ...
    //wangxingxing20131206 add for FR563019 start
    else if ((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == CHARGING_HOST))
    {
         if (BMT_status.temperature >= BATTERY_TEMPERATURE_P42)
         	{
			g_temp_CC_value = CHARGER_CURRENT_400;
		      if (Enable_BATDRV_LOG == 1) {
                     xlog_printk(ANDROID_LOG_DEBUG,"Power/Battery","[BATTERY] STANDARD_CHARGER or CHARGING_HOST CC mode charging : %d\r\n", CHARGER_CURRENT_400);
                   }  
         	}
	  else if(BMT_status.temperature >= BATTERY_TEMPERATURE_P40)
	  	{
			g_temp_CC_value = USB_CHARGER_CURRENT;
	           if (Enable_BATDRV_LOG == 1) {
                     xlog_printk(ANDROID_LOG_DEBUG,"Power/Battery","[BATTERY] STANDARD_CHARGER or CHARGING_HOST CC mode charging : %d\r\n", USB_CHARGER_CURRENT);
   ...
   //wangxingxing20131206 add for FR563019 end
	else 
	{
...
void pchr_turn_on_charging (void)
{
	...
	else
	{
		...
		{	
			...
			//wangxingxing20131206 add for FR563019 start
			if(BMT_status.temperature >= BATTERY_TEMPERATURE_P45)
				upmu_chr_vbat_cv_vth(0x18);                  //4.2V changed to 4.1V
			else
			      upmu_chr_vbat_cv_vth(0x1f);				// VBAT_CV_VTH    4.2changed to  4.1875
			//wangxingxing20131206 add for FR563019 end
...
{ 
    ...
    if (Enable_BATDRV_LOG >= 1) {
    	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:ADC] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld Current:%ld CAL:%d\n", BMT_status.charger_vol,
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	//g_sw_cv_enable=0;
 if((BMT_status.temperature>=(BATTERY_TEMPERATURE_P2))&&
		(BMT_status.temperature<=(BATTERY_TEMPERATURE_P50))&&
		(BMT_status.temperature != ERR_CHARGE_TEMPERATURE)&&
		(BMT_status.charger_protect_status == BATTERY_OVER_TEMP))
		{
		     BMT_status.bat_charging_state = CHR_PRE;
...
void mt_battery_notify_check(void)
{
 ...
 if( upmu_is_chr_det() == KAL_TRUE)
 	...
 	{
 	   if((BMT_status.temperature >= BATTERY_TEMPERATURE_P58) || (BMT_status.temperature <= BATTERY_TEMPERATURE_N18))
		//if(BMT_status.temperature > 20) //test
		{
			g_BatteryNotifyCode |= 0x0002;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] bat_temp(%d) > 50'C\n", BMT_status.temperature);
