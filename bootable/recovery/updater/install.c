...
Value* WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (!strcmp(partition, "logo") || !strcmp(partition, "preloader") || !strcmp(partition, "uboot") || !strcmp(partition, "dsp_bl") || !strcmp(partition, "bootimg")) {
        FILE *fp = fopen("/proc/dumchar_info", "r");
        if (fp) {
            char buf[128], p_name[32], p_size[32], p_addr[32], p_actname[32];
            int p_type = 0;
            ...
            while (fgets(buf, sizeof(buf), fp)) {
                if (sscanf(buf, "%s %s %s %d %s", p_name, p_size, p_addr, &p_type, p_actname) == 5) {
                    ...
                    if (strlen(p_name) && !strcmp(partition, p_name) && (p_type == 2)) {
                        char part_name[128];
