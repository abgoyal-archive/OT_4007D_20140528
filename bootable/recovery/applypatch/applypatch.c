...
int applypatch_check(const char* filename,
    ...
    if (LoadFileContents(filename, &file) != 0 ||
        ...
        if(strncmp(filename,"MTD:",4)==0 && num_patches==0)
        {/*
          if the filename is a MTD partition, the sha1 strings are embedded in the name,
          the parameter patch_sha1_str is empty
          we need to use these sha1 to check the cache file
          */
          int i;
          ...
          printf("check cache for %s\n",strtok(NULL,":")); /* the part name ex:FOTA */
          
          sha1_store.num=0;
          
          for(i=0;sha1_store.num<2;i++) {
            const char *str=strtok(NULL, ":"); /* find the src file size */
            if (str==NULL) 
              break;
            //printf("found %s\n",str);
            else if (i & 1) {
              if (strlen(str) != SHA_DIGEST_SIZE*2) {
                fprintf(stderr,"sha1 parse failed %s (should be %d digits)\n", str, SHA_DIGEST_SIZE*2);
                continue;
              }
              printf("use sha1 %s \n",str);
              sha1_store.val[sha1_store.num++] = str;
            }
          }
        }
        
        if (FindMatchingPatch(file.sha1, 
                              sha1_store.num ? sha1_store.val : patch_sha1_str, 
                              sha1_store.num ? sha1_store.num : num_patches) < 0) {
