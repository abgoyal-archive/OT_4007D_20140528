...
static void format_cache_partition(void)
{
    mtd_scan_partitions();
    const MtdPartition* mtd = mtd_find_partition_by_name("/cache");
    if (mtd == NULL) {
        fprintf(stderr, "Can't find cache partiton\n");
        goto done;
    }
    MtdWriteContext* ctx = mtd_write_partition(mtd);
    if (ctx == NULL) {
        fprintf(stderr, "Can't write cache partition\n");
        goto done;
    }
    if (mtd_erase_blocks(ctx, -1) == -1) {
        mtd_write_close(ctx);
        fprintf(stderr, "Failed to erase cache partition\n");
        goto done;
    }
    if (mtd_write_close(ctx) != 0) {
        fprintf(stderr, "Failed to close cache partition\n");
        goto done;
    }
...
install_package(const char *path)
{
	...
	//add for can't open /sdcard/update.zip, complain file not exist 
	do{
		DIR* d;
		struct dirent* de;
		char t[256];
